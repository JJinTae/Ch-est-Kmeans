...
{
    public class gen_Symbol
    {
        static Random rnd = new Random();
        public Complex Hk;
        public Complex[] Gen_S_Symbol(int qam, int SymbolNumber)    // S 심볼 생성(몇qam인지, 심볼의 갯수)
        {
            Complex[] ComArray = new Complex[SymbolNumber];         // Complex 배열 생성
            double Re, Im;                                          // 더블형의 실수부와 허수부
            ...
            if (qam==4)
            {
                ...
                for (int i = 0; i < SymbolNumber; i++)
                {
                    ...
                    // ComArray[i] = Complex.Multiply(new Complex(Re, Im), Hk);
                }
            }
            // 16QAM
            else if (qam == 16)
